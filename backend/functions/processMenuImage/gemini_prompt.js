...
/**
 * gemini_prompt.js — Gemini Prompt Template Module
 * -------------------------------------------------
 * Exports a function that builds the structured prompt for Vertex AI Gemini.
 * The prompt instructs the model to return ONLY valid JSON with no extra text.
 *
 * Supported dietary restriction types:
 ...
 * This is the "persona" that Gemini adopts for every request.
 *
 * @param {string[]} restrictions - User dietary restriction codes
 * @param {string}   nativeLanguage - BCP-47 language code (e.g. 'ca', 'en')
...
/**
 * Builds the complete user prompt that includes the OCR text
 * and specifies the exact output schema.
 *
 * @param {string}   ocrText        - Raw text extracted by Cloud Vision OCR
 * @param {string[]} restrictions   - User dietary restriction codes
 * @param {string}   nativeLanguage - BCP-47 language code
...
/**
 * Maps human-readable restriction names to their codes.
 * Used for display purposes in the Gemini prompt context.
 */
...
/**
 * All supported restriction type codes.
 */
const SUPPORTED_RESTRICTIONS = Object.keys(RESTRICTION_LABELS);

/**
 * Builds enriched restriction descriptions for the Gemini prompt.
 * Instead of just sending codes like "GLUTEN", we send
 * "GLUTEN: Gluten (wheat, barley, rye, oats)" for better AI understanding.
 *
 * @param {string[]} restrictionCodes - Array of restriction codes
 * @returns {string[]} Array of enriched restriction strings
 */
...
/**
 * Main export: builds the full prompt configuration for Gemini.
 *
 * @param {string}   ocrText        - Raw OCR text from Cloud Vision
 * @param {string[]} restrictions   - User dietary restriction codes
 * @param {string}   nativeLanguage - Target language code (BCP-47)
 * @returns {{ systemInstruction: string, userPrompt: string }}
 */
function buildGeminiPrompt(ocrText, restrictions = [], nativeLanguage = 'en') {
  // Enrich the restriction codes with descriptions for better AI understanding
